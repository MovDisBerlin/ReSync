...
def save_synchronized_recordings(
    ...
    if CROP_BOTH:
        if saving_format == 'csv':
            ...
            external_df_offset.to_csv(join(saving_path, 
                                           ('External_data_' 
                                            + str(session_ID) 
                                            + '.csv')), index=False
                                            )
        ...
        if saving_format == 'pickle':
            ...
            external_filename = join(
                saving_path, 
                ('External_data_' + str(session_ID) + '.pkl')
                )
        ...
        if saving_format == 'mat':
            ...
            external_filename = join(
                saving_path, 
                ('External_data_' + str(session_ID) + '.mat')
                )
            savemat(LFP_filename, 
                    {'data': LFP_df_offset.T,'fsample': sf_LFP, 
                     'label': np.array(LFP_df_offset.columns.tolist(), 
                                       dtype=object).reshape(-1,1)}
                                       )
            savemat(external_filename, 
        ...
        if saving_format == 'csv':
            ...
            external_df_offset.to_csv(join
                                      (saving_path, 
                                       ('External_data_' + str(session_ID) + '.csv')), 
                                       index=False
                                       )
